...
import type { ReviewState, ReviewStatus } from "./proposal-review.types";
import { getEffectiveStatus } from "./proposal-review.utils";

import type { ProposalScenario } from "./proposal-review.types";
...
function countStatuses(
  ...
  const counts: StatusCounts = { approved: 0, rejected: 0, pending: 0, modified: 0, total: scenarios.length };
  for (const s of scenarios) {
    const status = (overrides.get(s.id)?.review_status as ReviewStatus) ?? s.review_status;
    counts[status]++;
  }
  return counts;
}
...
function StatusCountBadges({ counts }: { counts: StatusCounts }) {
  return (
    <Stack direction="row" spacing={0.5} sx={{ flexShrink: 0, alignItems: "center" }}>
      {counts.approved > 0 && (
        <Typography component="span" sx={{ fontSize: 10, fontWeight: 700, color: "success.main" }}>
          {counts.approved}✓
        </Typography>
      )}
      {counts.rejected > 0 && (
        <Typography component="span" sx={{ fontSize: 10, fontWeight: 700, color: "error.main" }}>
          {counts.rejected}✗
        </Typography>
      )}
      {counts.pending > 0 && (
        <Typography component="span" sx={{ fontSize: 10, fontWeight: 600, color: "text.disabled" }}>
          {counts.pending}
        </Typography>
      )}
    </Stack>
  ...
  return (
    <Box>
      <List dense disablePadding>
        {proposal.test_plans.filter((p) => p.test_areas.some((a) => a.scenarios.length > 0)).map((plan) => {
          const planExpanded = expanded[plan.id] ?? false;
          const allPlanScenarios = plan.test_areas.flatMap((a) => a.scenarios);
          ...
          return (
            <Box key={plan.id}>
              ...
              >
                ...
                {planCounts.approved < planCounts.total && (
                  <Button
                    ...
                    sx={{ ml: 1.5, fontSize: 10, textTransform: "none", py: 0, px: 1, minWidth: "auto", lineHeight: 1.8 }}
                  >
                    {t("review.approveAll")}
                  </Button>
                )}
              ...
              <Collapse in={planExpanded}>
                ...
                >
                {plan.test_areas.filter((a) => a.scenarios.length > 0).map((area, areaIdx, filteredAreas) => {
                  const areaExpanded = expanded[area.id] ?? false;
                  const isLast = areaIdx === filteredAreas.length - 1;
                  const areaCounts = countStatuses(area.scenarios, overrides);
                    ...
                    >
                      <Box
                        sx={{
                          ...
                          zIndex: 2,
                          bgcolor: "background.paper",
                          boxShadow: "0 1px 0 0 var(--mui-palette-grey-200)",
                        }}
                      >
                        <Box
                          sx={{
                        ...
                        <ListItemButton
                          ...
                          onClick={() => {
                            dispatch({ type: "TOGGLE_AREA", planId: plan.id, areaId: area.id });
                            dispatch({
                              type: "SELECT_AREA",
                              planId: plan.id,
                              areaId: area.id,
                        ...
                        <ChecklistRtlIcon
                          sx={{
                            fontSize: 18,
                            mr: 0.75,
                            color: areaExpanded ? "primary.main" : "text.secondary",
                          }}
                        />
                        <ListItemText
                          primary={
