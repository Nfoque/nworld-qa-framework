...
const QAAP_DATA = {
  user: { name: 'Elena García', email: 'elena.garcia@nfq.com', role: 'QA Lead', initials: 'EG' },
  tenant: { name: 'Nfq', slug: 'nfq', logo: 'assets/nfq-full-logo.png' },
  ...
  Scenario: Successful checkout with credit card
    Given I am logged in as "testuser@nfq.com"
...
test.describe('Checkout Flow', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.fill('[data-testid="email"]', 'testuser@nfq.com');
  ...
  chatMessages: [
    { role:'system', content:'Plan loaded: Checkout Flow E2E — 24 scenarios, 18 approved, 4 pending, 2 rejected. Sources: Jira (TM-Sprint-47), GitHub (nfq/web-frontend), OpenAPI spec v3.1.' },
...
+     await $('~login-email').setValue('testuser@nfq.com');
...
+ await injectAuthToken(page);` },
  ...
  deliveryConfigs: [
    { id:'del-001', name:'Nightly QA Report', trigger:'After every nightly regression', recipients:['qa-team@nfq.com', '#qa-channel (Slack)'], format:'HTML', plans:['Checkout Flow E2E', 'Payment Gateway API'], enabled:true },
    { id:'del-002', name:'Weekly Summary to Management', trigger:'Every Monday 09:00', recipients:['tech-leads@nfq.com'], format:'PDF', plans:['All plans'], enabled:true },
  ...
  // ─── Connectors Data ───
  connectors: [
    { id:'con-001', name:'Jira + XRay', type:'task_tracker', icon:'jira', status:'connected', description:'Test management, execution tracking & requirements traceability', config:{ project:'TM', board:'Sprint Board', url:'nfq.atlassian.net', xrayCloud:'Enabled', testExecutionIssueType:'Test Execution' }, lastSync:'5m ago' },
    { id:'con-002', name:'GitHub + Actions', type:'code_repo', icon:'git', status:'connected', description:'Source code, PR triggers & CI/CD pipeline integration', config:{ org:'nfq', repos:['web-frontend','api-gateway','payment-service','mob-app-ios'], url:'github.com/nfq', actionsWorkflow:'qa-regression.yml', prTrigger:'Enabled' }, lastSync:'2m ago' },
    { id:'con-003', name:'AWS S3', type:'storage', icon:'doc', status:'connected', description:'Test artifacts, screenshots & documentation', config:{ bucket:'nfq-qa-docs', region:'eu-west-1' }, lastSync:'1h ago' },
    ...
    { id:'con-005', name:'Slack', type:'notification', icon:'chat', status:'connected', description:'Notifications & alerts by vertical', config:{ workspace:'Nfq', channels:['#qa-alerts','#qa-checkout','#qa-payments','#qa-auth'] }, lastSync:'Active' },
    ...
    { id:'con-007', name:'Gmail API', type:'external', icon:'send', status:'connected', description:'OTP verification & transactional email testing', config:{ serviceAccount:'qa-bot@nfq.iam.gserviceaccount.com', scopes:'gmail.readonly' }, lastSync:'Active' },
  ...
  teamMembers: [
    { name:'Elena García', email:'elena.garcia@nfq.com', role:'Admin', status:'active', lastActive:'Just now', initials:'EG' },
    { name:'Carlos Ruiz', email:'carlos.ruiz@nfq.com', role:'QA Expert', status:'active', lastActive:'2h ago', initials:'CR' },
    { name:'Marta López', email:'marta.lopez@nfq.com', role:'QA Expert', status:'active', lastActive:'1d ago', initials:'ML' },
    { name:'David Chen', email:'david.chen@nfq.com', role:'Viewer', status:'active', lastActive:'3d ago', initials:'DC' },
    { name:'Sophie Martin', email:'sophie.martin@nfq.com', role:'QA Expert', status:'pending', lastActive:'Invited', initials:'SM' },
  ],

  ssoProviders: [
    { name:'Microsoft Azure AD', status:'active', protocol:'SAML 2.0', domain:'nfq.com', lastLogin:'Today 14:32', users:24 },
    { name:'Google Workspace', status:'inactive', protocol:'OIDC', domain:'nfq.com', lastLogin:'Never', users:0 },
  ],

  notificationChannels: [
    { id:'email', name:'Email', icon:'send', status:'connected', config:'SMTP configured via SendGrid', recipients:3 },
    { id:'slack', name:'Slack', icon:'chat', status:'connected', config:'Connected to Nfq workspace', recipients:2 },
    { id:'teams', name:'Microsoft Teams', icon:'chat', status:'not_configured', config:null, recipients:0 },
    { id:'webhook', name:'Webhook', icon:'connectors', status:'connected', config:'https://hooks.nfq.com/qa-alerts', recipients:1 },
