...
interface GitHubConnectorDialogProps {
  connector: Connector;
  onClose: () => void;
}

export function GitHubConnectorDialog({
  connector,
  onClose,
}: GitHubConnectorDialogProps) {
  ...
  useEffect(() => {
    ...
    testConnector.mutate(
      ...
      {
        onSuccess: (data) => {
          const result = data.result as unknown as GitHubTestResult;
          if (result.tokenValid) {
            setValidatedUser(result.user ?? null);
  ...
  const testResult = testConnector.data?.result as GitHubTestResult | undefined;
    ...
    <>
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          {testConnector.isSuccess && !testResult?.tokenValid && (
            <Alert severity="error" sx={{ fontSize: 13 }}>
